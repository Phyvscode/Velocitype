...
// 4. Change pt-24 px-8 to pt-12 pb-24 px-8
boxTs = boxTs.replace(
  /className="flex-1 pt-24 px-8 overflow-hidden flex flex-col"/g,
  `className="flex-1 pt-24 pb-24 px-8 overflow-hidden flex flex-col"`
);

// 5. Inject Global Timer Overlay right after `<div className="flex-1 w-full flex h-full">`
const timerOverlay = `
...
boxTs = boxTs.replace(
  ...
  `<div className="flex-1 w-full flex h-full relative">${timerOverlay}`
);
